...
public class HeaderMenu extends BaseComponent<HeaderMenu> {
    ...
    public UserProfilePage clickOnProfileButton() {
        getSelf().$("a[href='/profile']").shouldBe(visible).click();
        return new UserProfilePage();
    }

    @Step("Нажатие на кнопку 'Friends'")
    public FriendsPage clickOnFriendsButton() {
        getSelf().$("a[href='/people/friends']").shouldBe(visible).click();
        return new FriendsPage();
    }

    @Step("Нажатие на кнопку 'All people'")
    public PeoplePage clickOnAllPeopleButton() {
        getSelf().$("a[href='/people/all']").shouldBe(visible).click();
        return new PeoplePage();
    }

    @Step("Нажатие на кнопку 'Sign out'")
    public LoginPage clickOnSignOutButton() {
        getSelf().$x("//li[text()='Sign out']").shouldBe(visible).click();
